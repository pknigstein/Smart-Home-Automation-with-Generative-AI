...
class Fan:
    ...
    def set_speed(self, speed: str) -> str:
        if not self.is_on:
            raise ValueError("The fan must be ON before its speed can be changed.")
...
class Thermostat:
    ...
    def increase_temperature(self, amount: int = 1) -> str:
        self.temperature = min(self.MAX_TEMPERATURE, self.temperature + amount)
        return f"{self.name} thermostat set to {self.temperature}°C."

    def decrease_temperature(self, amount: int = 1) -> str:
        self.temperature = max(self.MIN_TEMPERATURE, self.temperature - amount)
        return f"{self.name} thermostat set to {self.temperature}°C."
