...
class SmartHomeController:
    ...
    def _execute(
        ...
        if command.action is None:
            return "Please specify your command", None
        ...
        try:
            if command.action == "turn_on":
                if isinstance(device, Fan) and isinstance(command.value, str):
                    device.turn_on(command.value)
                else:
                    device.turn_on()
                status = device.get_status()
                return self._build_turn_on_feedback(device_name, status), status
            ...
            if command.action == "increase_temperature":
                if not isinstance(device, Thermostat):
                    return "Please specify your command", None
                device.increase_temperature(1)
                status = device.get_status()
                return self._build_temperature_feedback(device.temperature), status

            if command.action == "decrease_temperature":
                if not isinstance(device, Thermostat):
                    return "Please specify your command", None
                device.decrease_temperature(1)
                status = device.get_status()
                return self._build_temperature_feedback(device.temperature), status
    ...
    def _build_turn_on_feedback(
        self, device_name: str | None, status: dict[str, Any]
    ) -> str:
        if device_name == "fan":
            return f"The fan is now ON at {status['speed']} speed."
        return self._build_power_feedback(device_name, True)
    ...
    def _build_status_feedback(
        ...
        if device_name == "fan":
            if status["power"] == "off":
                return "The fan is currently OFF and its speed is n/a."
            return (
    ...
    def _build_overall_status_feedback(self, statuses: dict[str, dict[str, Any]]) -> str:
        ...
        fan_summary = (
            "Fan: OFF with speed n/a."
            if fan_status == "OFF"
            else f"Fan: {fan_status} at {fan_speed} speed."
        )
        return (
